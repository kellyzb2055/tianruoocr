...
- **win7** 系统有些 `AI` 接口无法使用，报错：`请求被中止：未能创建 SSL/TLS 安全通道`。

  遇此错误，请先尝试开启 `TLS1.2` 协议，如果开启之后依旧报此错误，则很难再解决。
