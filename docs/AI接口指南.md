...
下面是 **`win7`** 看的，非 **`win7`** 不用看

### 特别（win7 系统）

经过 `win7 x64` 虚拟机测试：

- **win7** 系统部分 `AI` 接口无法使用，报错：`请求被中止：未能创建 SSL/TLS 安全通道`。

  遇此错误，请先尝试开启 `TLS1.2` 协议，如果开启之后依旧此错：

  - **此错误的原因**

  `Windows 7` 操作系统原生支持的 `SSL/TLS` 加密套件（Cipher Suites）版本过旧且缺失。

  当程序尝试与采用现代安全标准的服务器建立连接时，由于操作系统底层（`SChannel`）无法提供服务器要求的加密算法（如 ECDHE-GCM 或 ChaCha20），导致 SSL 握手阶段协商失败，连接被迫中止。

  - **解决办法**
  
  使用 https://www.ssllabs.com/ssltest/analyze.html 输入 `api` 地址，查看 `api` 服务器支持的 **`tls` 协议和加密算法**。
  
  然后**检查当前系统开启的 `tls` 协议是否被 `api` 服务器支持**，
  
  **检查当前系统的加密套件是否含有任意一个 `api` 服务器需要的加密算法**。(**IIS Crypto**这个软件可以查看系统的协议及加密套件, 不用 IIS Crypto, 用其他办法查看也行 ）
  
  如果系统加密套件缺少 `api` 服务器支持的加密算法，需要下载 **`windows` 更新补丁** 增加系统对加密算法的支持。
  
  对应的补丁请自行查找，可以优先检索 `Github issue` 查看有无同样系统案例
  
  > 如果找不到补丁，下面有一个麻烦的办法：
  >
  > 使用 `Nginx` 反向代理 **` AI 接口地址`**：
  >
  > 利用 `Nginx` 本地反向代理绕过 `Win7 SSL` 限制，配置 `Nginx` 监听本地端口并转发请求至目标 AI 接口地址，同时将软件设置里的 AI 接口地址修改为 `http://localhost:端口`。
  >
  > **此方法不能一劳永逸，还是推荐去寻找补丁。**
  >
  > **如果你有更好的方法，欢迎提交**
...
- 如果你 **设置好模式文件的路径后，又更改了模式文件的内容**，
  ...
  请重新打开一次 **设置** 然后关闭，或者 **重启软件**，或者去设置里重新浏览选择模式文件，不然更改不会生效
