...
- `PaddleOCR2` 需要下载补丁包才能正常使用，官方说明：

  ```markdown
  如果您使用的是Win7-x64，并且您的CPU确实支持AVX2，那么您可能还需要将以下3个DLL提取到C:\Windows\System32文件夹中以使其运行：
  ...
  您可以在此处下载这3个DLL:
  <https://github.com/sdcb/PaddleSharp/files/10110622/win7-x64-onnxruntime-missing-dlls.zip>
  ```

- `RapidOCR` 不支持 win7 系统，暂时无解，win 7 32 位应该也不行
