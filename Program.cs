    ...
    internal static class Program
    {
        ...
        public static void CheckUpdate()
        {
            try
            {
                ...
                // 2. 准备 API URL
                string apiUrl = checkPreRelease
                    ? "https://gh-proxy.com/https://api.github.com/repos/Topkill/tianruoocr/releases"
                    : "https://gh-proxy.com/https://api.github.com/repos/Topkill/tianruoocr/releases/latest";

                var request = System.Net.WebRequest.Create(apiUrl) as System.Net.HttpWebRequest;
                request.UserAgent = "TianruoOCR";
                request.Accept = "application/vnd.github.v3+json";
                request.Timeout = 10000;

                // --- ETag 缓存准备 ---
                string suffix = checkPreRelease ? "_Releases" : "_Latest";
                string etagKey = "ETag" + suffix;
                string versionKey = "CachedVersion" + suffix;
                string urlKey = "CachedUrl" + suffix;
                ...
                using (var response = request.GetResponse())
                {
                    ...
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        if (checkPreRelease)
                        {
                            ...
                            if (jsonArray.Count == 0) { CommonHelper.ShowHelpMsg("未找到任何发行版本。"); return; }
                        ...
                        // --- 【关键修改】智能匹配下载链接 ---
                        string downloadUrl = GetBestMatchAsset(assets);

                        // --- 缓存逻辑 ---
                        // 优先缓存文件直链，没有则缓存网页链接
                        string targetUrl = !string.IsNullOrEmpty(downloadUrl) ? downloadUrl : htmlUrl;

                        string newEtag = response.Headers["ETag"];
                        if (!string.IsNullOrEmpty(newEtag))
                        {
                            IniHelper.SetValue("更新", etagKey, newEtag);
                            IniHelper.SetValue("更新", versionKey, newVersion);
                            IniHelper.SetValue("更新", urlKey, targetUrl);
                        }
                        ...
                        ShowUpdateDialog(newVersion, curVersion, isPreRelease, body, htmlUrl, downloadUrl);
                    }
                }
            }
            catch (System.Net.WebException ex)
            {
                // === 场景 B: 服务器返回 304 Not Modified ===
                if (ex.Response is HttpWebResponse httpResponse && httpResponse.StatusCode == HttpStatusCode.NotModified)
                {
                    Debug.WriteLine("ETag matched (304). Checking local cache...");

                    string suffix = Convert.ToBoolean(IniHelper.GetValue("更新", "CheckPreRelease") == "True" ? "True" : "False") ? "_Releases" : "_Latest";
                    string cachedVersion = IniHelper.GetValue("更新", "CachedVersion" + suffix);
                    string cachedUrl = IniHelper.GetValue("更新", "CachedUrl" + suffix);
                    string curVersion = Application.ProductVersion;

                    if (!string.IsNullOrEmpty(cachedVersion) && cachedVersion != "发生错误")
                    {
                        if (CheckVersion(cachedVersion, curVersion))
                        {
                            string msg = $"发现新版本：{cachedVersion}\n(检测到您上次未完成更新)\n\n当前版本：{curVersion}\n\n是否前往下载页面？";
                            if (MessageBox.Show(msg, "发现新版本", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                            {
                                Process.Start(string.IsNullOrEmpty(cachedUrl) || cachedUrl == "发生错误" ? "https://github.com/Topkill/tianruoocr/releases" : cachedUrl);
                            }
                            return;
                        }
                    }
                ...
                if (ex.Status == System.Net.WebExceptionStatus.Timeout) return;
        ...
        /// <summary>
        /// 【新增方法】根据当前进程架构（x86/x64）智能筛选最佳的下载文件
        /// </summary>
        private static string GetBestMatchAsset(JArray assets)
        {
            ...
            // 定义关键词
            string[] targetArchKeywords = is64Bit ? new[] { "x64", "win64", "64bit" } : new[] { "x86", "win32", "32bit" };
            string[] avoidArchKeywords = is64Bit ? new[] { "x86", "win32", "32bit" } : new[] { "x64", "win64", "64bit" };

            string bestUrl = null;
            int bestScore = -1; // 分数越高越匹配
            ...
            foreach (var asset in assets)
            {
                ...
                // 规则1：包含目标架构关键词 (+10分)
                if (System.Linq.Enumerable.Any(targetArchKeywords, k => nameLower.Contains(k)))
                {
                    currentScore += 10;
                }

                // 规则2：包含需避开的架构关键词 (-100分，极力避免)
                // 例如：我是x64，文件名里有x86，这通常不是我要的，除非它同时包含x64(万一有这种怪名)
                if (System.Linq.Enumerable.Any(avoidArchKeywords, k => nameLower.Contains(k)))
                {
                    currentScore -= 100;
                }

                // 规则3：如果不包含任何架构词，算作通用包 (+5分)
                bool hasAnyTarget = System.Linq.Enumerable.Any(targetArchKeywords, k => nameLower.Contains(k));
                bool hasAnyAvoid = System.Linq.Enumerable.Any(avoidArchKeywords, k => nameLower.Contains(k));
                if (!hasAnyTarget && !hasAnyAvoid)
                {
                    currentScore += 5;
                }
            ...
            // 如果所有文件都被排除（比如只有 update.json 这种文本文件），或者 assets 为空
            // 此时 bestUrl 可能为 null，上层代码会回退到使用 htmlUrl
            return bestUrl;
        }

        private static void ShowUpdateDialog(string newVersion, string curVersion, bool isPreRelease, string body, string htmlUrl, string downloadUrl)
        {
            ...
            if (isPreRelease) message += "⚠️ 这是一个预览版本，可能存在不稳定情况。\n\n";
            message += $"更新内容：\n{body}\n\n";
            message += "是否前往下载页面？";
